...
The coefficients of the fitted model are shown in @tbl-fixed-coefs-m4, and the random intercepts are shown in @tbl-random-coefs-m4. Because a random effect term is included (for gene line), then the intercept for each gene line is calculated as the average intercept (in @tbl-fixed-coefs-m4) plus the gene line specific intercept in @tbl-random-coefs-m4. The gene line specific intercept is due to the gene line random effect included in the chosen model. For some other gene line not included in the data, the average intercept can be used to estimate gene expression.
