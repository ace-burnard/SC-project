...
ranova(m2) %>% 
  ...
  # fmt_scientific(columns = 6, decimals = 4)
```
...
fixef(m4) %>% 
  data.frame() %>% 
  # mutate(names = rownames(data.frame(fixef(m4)))) %>%
  rename(value = ".") %>%
...
random_effects %>%
  ...
  # mutate(names = rownames(random_effects)) %>% 
