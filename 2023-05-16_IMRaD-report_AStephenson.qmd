...
@tbl-sjPlot-m23 shows the estimates for the first two mized effects models, as well as the p-values for these estimates. From this table, it can be seen that the cell line predictor is not statistically significant, so this term should be removed, which results in the model having only concentration and treatment (and the interaction term) as fixed effects.
