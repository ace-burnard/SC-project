...
The coefficients of the fitted model are shown in @tbl-fixed-coefs-m4, where the intercept for each gene line 
...
random_effects %>%
  ...
  pivot_wider(names_from = names, values_from = `(Intercept)`) %>% 
```
