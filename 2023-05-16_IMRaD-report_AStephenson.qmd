...
The fitted model has the coefficients shown in **TABLE HERE**
...
for (i in 1:length(orig_rows)) {
  row.names(coefs)[i] <- paste(rownames(coefs)[i], "(Intercept)", sep = '')
  coefs[[1]][i] <- coefs[[1]][i] + fixed_effects[[1]]
}
for (i in 2:8) {
  coefs <- coefs %>% 
    add_row(value = fixed_effects[[i]])
  row.names(coefs)[length(orig_rows)+(i-1)] <- names(fixed_effects)[i]
}
```
