...
These three models can be compared two each other and to the linear model without random effects using AIC values, R$^{2}$ values and RMSE values (shown in @tbl-performance-m234).
...
```{r fig-ge-conc-grouping-model, fig.cap = "A plot of gene expression as a function of concentration, coloured by cell line (wild-type or cell-type 101) and treatment (placebo or activating factor 42) as well as gene line, with the fitted model indicated by the lines."}
data_long %>%
