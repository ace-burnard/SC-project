...
@tbl-ranova-m2 and @tbl-ranova-m4 show the results of applying the function `ranova` to the two mixed effects models considered. From these tables, it can be seen that the random effects are significant in both models. Thus, these random effects terms should not be removed.
...
These three models can be compared to each other and to the linear model without random effects using AIC values, R$^{2}$ values and RMSE values (shown in @tbl-performance-m234).
